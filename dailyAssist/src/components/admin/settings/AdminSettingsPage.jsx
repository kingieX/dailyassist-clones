...
const TABS = ["Profile settings", "Notification settings", "System Log"];
    ...
    <div className="flex flex-col">
      ...
      <Section label="Personal Information" sub="Update your personal details here.">
        <div className="flex flex-col gap-5">
          ...
          {/* Staff Role — disabled with warning */}
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-start gap-2 mt-1">
              ...
              <p className="text-xs text-gray-500">Your role is managed by a Super Admin. Contact your Super Admin to request a role change.</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button
              onClick={() => { setFirstName(user?.name?.split(" ")[0] ?? ""); setLastName(user?.name?.split(" ")[1] ?? ""); }}
...
function NotificationSettingsTab() {
  const initialSettings = Object.fromEntries(
    adminNotificationItems.map((item) => [item.key, item.defaultValue])
  );
  ...
  return (
    <div className="flex flex-col py-2">
      {adminNotificationItems.map(({ key, label, sub }, idx) => (
        <div key={key}
          className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-6 px-2
                      ${idx !== adminNotificationItems.length - 1 ? "border-b border-gray-100" : ""}`}>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            <button onClick={() => toggle(key)}
              className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${settings[key] ? "bg-amber-400" : "bg-gray-200"}`}>
              <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-200 ${settings[key] ? "left-6" : "left-1"}`} />
            </button>
            <span className={`text-sm font-medium ${settings[key] ? "text-gray-700" : "text-gray-400"}`}>
              {settings[key] ? "On" : "Off"}
            </span>
          </div>
        </div>
...
function SystemLogTab() {
  ...
  const exportCSV = () => {
    const header = "Time,User,Action,Module,Description,Status";
    const rows   = filtered.map((l) => `${l.time},${l.user},${l.action},${l.module},"${l.description}",${l.status}`);
