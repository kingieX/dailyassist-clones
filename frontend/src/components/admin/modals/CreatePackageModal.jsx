...
// import { X, Smile, ChevronDown, Plus, Trash2 } from "lucide-react";
...
// export default function CreatePackageModal({ isOpen, onClose }) {
...
//   const [showingCharge, setShowingCharge] = useState(false);
...
//             <h2 className="text-xl font-bold text-gray-900">Create a new package</h2>
...
//                 >
//                   <span className={icon ? "text-gray-700" : ""}>{icon || "Pick an icon"}</span>
...
//                   Features{" "}
//                   <span className="text-gray-400 font-normal">(10 max)</span>
//                 </label>
//                 {features.length < 10 && (
//                   <button
...
//                     autoFocus
//                     className="flex-1 bg-gray-100 border border-gray-200 rounded-xl px-4 py-2.5
//                                text-sm text-gray-700 outline-none focus:ring-2 focus:ring-amber-200
//                                placeholder-gray-400" />
...
//               </div>
//               {showingCharge && (
//                 <input type="text" value={additionalCharge}
//                   onChange={(e) => setAdditionalCharge(e.target.value)}
//                   placeholder="e.g. Transport mileage: 45p/mile"
//                   className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-3
//                              text-sm text-gray-700 outline-none focus:ring-2 focus:ring-amber-200
//                              placeholder-gray-400" />
//               )}
...
//             onClick={(e) => e.stopPropagation()}
//           >
//             <h2 className="text-xl font-bold text-gray-900">Create this package?</h2>
//             <p className="text-sm text-gray-500 leading-relaxed">
//               You're about to create a new package in Daily Assist.
//               Please confirm that the package details are correct before proceeding.
//             </p>
...
import { useState } from "react";
import { X, Smile, ChevronDown, Trash2 } from "lucide-react";
import CheckInSuccessModal from "../../modals/CheckInSuccessModal";
...
export default function CreatePackageModal({ isOpen, onClose, onSuccess }) {
  const [icon, setIcon]                 = useState("");
  const [iconOpen, setIconOpen]         = useState(false);
  const [name, setName]                 = useState("");
  const [price, setPrice]               = useState("");
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setApiError(err.response?.data?.message || "Failed to create package. Please try again.");
      setShowConfirm(false);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-start justify-center
                     bg-black/40 px-3 sm:px-6 py-4 sm:py-6 overflow-y-auto"
        onClick={onClose}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-auto flex flex-col my-2"
          onClick={(e) => e.stopPropagation()}>
          ...
          <div className="px-6 py-5 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <button onClick={() => setIconOpen(!iconOpen)}
                  className="w-full flex items-center justify-between bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-400 hover:bg-gray-200 transition-colors">
                  <span className={icon ? "text-gray-700" : ""}>{icon || "Pick an icon"}</span>
                  <Smile className="w-5 h-5 text-gray-400" />
                </button>
                {iconOpen && (
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Package name"
                className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-amber-200 placeholder-gray-400" />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <button onClick={() => setDurationOpen(!durationOpen)}
                  className="w-full flex items-center justify-between bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm hover:bg-gray-200 transition-colors">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea value={tagline} onChange={(e) => setTagline(e.target.value)} placeholder="Type tagline..." rows={3}
                className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-amber-200 placeholder-gray-400 resize-none" />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {features.map((f, i) => (
                <div key={i} className="flex items-center gap-2 bg-gray-50 rounded-xl px-4 py-2.5">
                  <span className="flex-1 text-sm text-gray-700">{f}</span>
                  <button onClick={() => removeFeature(i)} className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-red-400 transition-colors">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              ))}
              {addingFeature && (
                <div className="flex gap-2">
                  <input type="text" value={newFeature} onChange={(e) => setNewFeature(e.target.value)}
                    ...
                    className="flex-1 bg-gray-100 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-amber-200 placeholder-gray-400" />
                  <button onClick={addFeature} className="px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-900 transition-colors hover:opacity-90" style={{ backgroundColor: AMBER }}>Add</button>
                  <button onClick={() => { setAddingFeature(false); setNewFeature(""); }} className="px-4 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-gray-800">Additional Charge</label>
                {!showingCharge && (
                  <button onClick={() => setShowingCharge(true)} className="text-sm font-semibold" style={{ color: AMBER }}>+ Add</button>
                )}
              </div>
              {showingCharge && (
                <input type="text" value={additionalCharge} onChange={(e) => setAdditionalCharge(e.target.value)}
                  placeholder="e.g. Transport mileage: 45p/mile"
                  className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-amber-200 placeholder-gray-400" />
              )}
            </div>
          ...
          <div className="px-6 pb-6 pt-2 flex gap-3">
            ...
            <button onClick={() => setShowConfirm(true)} className="flex-1 py-3.5 rounded-xl font-semibold text-sm text-gray-900 transition-colors hover:opacity-90" style={{ backgroundColor: AMBER }}>Create Package</button>
          </div>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 px-4" onClick={() => setShowConfirm(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm px-8 py-8 flex flex-col items-center gap-4 text-center" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-xl font-bold text-gray-900">Create this package?</h2>
            <p className="text-sm text-gray-500 leading-relaxed">You're about to create a new package in Daily Assist. Please confirm that the package details are correct before proceeding.</p>
            <div className="flex gap-3 w-full mt-2">
              <button onClick={() => setShowConfirm(false)} className="flex-1 py-3 rounded-xl border font-medium text-sm transition-colors hover:opacity-90" style={{ borderColor: AMBER, color: "#c8860a" }}>Cancel</button>
              <button onClick={handleCreate} disabled={submitting} className="flex-1 py-3 rounded-xl font-semibold text-sm text-gray-900 transition-colors hover:opacity-90 disabled:opacity-60" style={{ backgroundColor: AMBER }}>
                {submitting ? "Creating..." : "Yes, Create Package"}
              </button>
            </div>
          </div>
        </div>
